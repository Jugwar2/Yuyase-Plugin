...
## [1.7.0] - 2026-07-26 (stable)

### Added — Canonical pipeline (franchise-graph matching)

Retrofits the four-invariant architecture onto the existing sources, fixing the
long-standing **sibling-franchise leak** (e.g. searching for *Uma Musume:
Pretty Derby S2* turned up *Uma Musume: Cinderella Gray*, *Road to the Top*,
and the *Beginning of a New Era* movie).

- `src/lib/resolver.js` — Canonical Resolver + Franchise Graph. Walks AniList
  GraphQL relations to enumerate every branch of a franchise (TV seasons,
  movies, OVAs, side stories), serializes the SEQUEL/PREQUEL chain to assign
  `seasonInFranchise` per branch, and produces a single canonical entity that
  downstream filters consult.
- `src/lib/normalize.js` — NormalizedTorrent schema + branch-signature
  extractor. Every release is parsed once into a uniform object; no downstream
  module inspects raw tracker titles.
- `src/lib/filters.js` — Hard validation gates (reject, not score):
  `gateFranchise`, `gateBranch`, `gateMovieVsTv`, `gateOvaVsSeason`,
  `gateEpisodeRange`, `gateAirDate`. Impossible candidates are dropped before
  the scorer.
- `src/lib/scorer.js` — Bayesian-flavoured scorer (alias, episode, season,
  release-group, recency, seeders, parser-confidence weighting). Ranks only
  candidates that survived the gates.
- `src/lib/shared.js` — exposes `shapeAny` / `canonicalShapeAll` async entry
  that picks the new pipeline when the query carries an `anilistId`, otherwise
  the legacy synchronous `shapeAll`. Sources now call `shapeAny`.
- `test/franchise.test.mjs` — offline regression test for the Umamusume leak.

### Changed

- `src/nyaa.js`, `src/animetosho.js`, `src/toonshub.js`, `src/yameii.js` —
  call `shapeAny` (async) instead of the synchronous `shapeAll`.
- `test/relevance.test.mjs` — now passes `anilistId` per show to exercise the
  new pipeline end-to-end, and adds a *Uma Musume S2* case which asserts zero
  Cinderella Gray / Road to the Top / New Era leakage.
- `hayase/index.json` — bumped Nyaa, AnimeTosho, Yameii, ToonsHub to `1.1.0`.

### Behaviour

- Queries without an AniList ID are unchanged (legacy token path).
- Queries with an AniList ID now go through the new pipeline; if AniList
  GraphQL is unreachable the pipeline degrades to legacy `shapeAll` so searches
  keep working.
