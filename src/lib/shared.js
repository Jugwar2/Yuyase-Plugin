...
// ─── New canonical/filters/scorer pipeline ────────────────────────────
// Invariants 1-3 from the architecture doc: canonical entity → hard gates →
// Bayesian ranker. Sources opt-in when they have an AniList ID; the legacy
// token path (shapeAll/finalize) still handles anything without one (e.g.
// SubsPlease's own EP-id lookup) to stay a safe swap-in.
import { attachCanonical } from './resolver.js'
import { normalizeTorrent } from './normalize.js'
...
// Convert a NormalizedTorrent back into the legacy result shape Hayase already
// consumes. Delegates to the unified formatter so every source produces
// identical result objects, then stamps internal fields (_score, _tier) on top.
...
function accuracyBand (score, norm, sourceDefault) {
  // Higher score -> keep source default. Below a soft floor -> 'low'. Purely a
  // presentation band, never used to reject (gates already did that).
  if (score >= 12 || (score >= 6 && norm.confidence >= 0.7)) return sourceDefault
  if (score >= 3) return 'medium'
  return 'low'
}

// Tier letter for the legacy sortResults/finalize helpers. Run AFTER ranking
// so we can call the top quartile 'A', middle 'B', rest 'C' — mirrors the old
// tier A/B/C contract while being driven by real score instead of regex-only.
...
// Async entry point used by sources. Resolves the canonical entity for the
// query's anilistId, then runs every raw result through the normalize-validate-
// score pipeline and returns results in the legacy shape. Falls back to the
// legacy shapeAll pipeline when:
//   - no anilistId was supplied
//   - canonical entity could not be resolved (network, unknown id)
//   - every result failed a gate (degrades gracefully to old behaviour)
export async function canonicalShapeAll (items, ctx, sourceDefault, query) {
  ...
  if (!norms.length) {
    // Whole batch rejected by gates. Degrade to legacy path so the user still
    // sees something when our new opinion was too aggressive.
    return shapeAll(items, ctx, sourceDefault)
  }
...
// Adapter: prefer canonicalShapeAll when an anilistId is available, otherwise
// the synchronous shapeAll. This is the single entry sources should call
// instead of shapeAll — it returns either a Promise<results[]> (canonical) or
// results[] (legacy).
export async function shapeAny (items, ctx, sourceDefault, query) {
