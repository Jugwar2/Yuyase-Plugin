import {
  ...
  httpGet, checkNyaaFeed, buildQueries, searchContext, shapeAny, finalize,
...
function itemToResult (raw, opts) {
  ...
  const uploader = pickTag(raw, 'dc:creator') || ''

  return formatResult({
    title, hash, link: hash, seeders, leechers, downloads, size, date, uploader,
    tracker: PROFILE.name, accuracy: PROFILE.accuracy
