...
function entryToResults (entry, opts) {
  ...
  for (const dl of downloads) {
    ...
    out.push(formatResult({
      title, link: dl.magnet, hash, seeders: 0, leechers: 0, downloads: 0, size, date,
      uploader: 'SubsPlease', tracker: PROFILE.name,
      accuracy: opts.batch ? 'low' : tagAccuracy(opts.tier, date.getTime(), PROFILE.accuracy)
    }, PROFILE, { useMagnetRaw: true, batch: (opts.batch || opts.tier === 'B') }))
  }
  return out
}
